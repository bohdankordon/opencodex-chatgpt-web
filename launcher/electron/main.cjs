...
function buildUpgradeCompletionPatch({ upgrade, snapshotConfig }) {
  return {
    ...
    experimentalFreshConversationPerTurn: snapshotConfig?.experimentalFreshConversationPerTurn === true,
    useSavedChats: snapshotConfig?.useSavedChats === true,
...
function registerIpc({ logger, stateStore }) {
  ...
  handle("launcher:fresh-conversation-per-turn", async (_event, enabled, options) => {
    const ownership = resolveSetupOwnership(extractRequestedIntegrationMode(options), "fresh-conversation-per-turn");
    if (browserHost.activeTraceId || browserHost.currentOperation()) {
      throw new Error("Finish or cancel active ChatGPT turns before changing browser conversation retention");
    }
    const repairValidation = createRepairRuntimeValidation(ownership, "fresh-conversation-per-turn");
    await runtimeHost.setFreshConversationPerTurn(enabled, { integrationMode: ownership.integrationMode }, ownership.expectation, repairValidation);
    return syncFreshConversationPreference(stateStore, runtimeHost.runtimeConfigSnapshot().config);
  });
  handle("launcher:use-saved-chats", async (_event, enabled, options) => {
    const ownership = resolveSetupOwnership(extractRequestedIntegrationMode(options), "use-saved-chats");
    if (browserHost.activeTraceId || browserHost.currentOperation()) {
      throw new Error("Finish or cancel active ChatGPT turns before changing saved chats");
    }
    const repairValidation = createRepairRuntimeValidation(ownership, "use-saved-chats");
    await runtimeHost.setUseSavedChats(enabled, { integrationMode: ownership.integrationMode }, ownership.expectation, repairValidation);
    return syncFreshConversationPreference(stateStore, runtimeHost.runtimeConfigSnapshot().config);
  });
