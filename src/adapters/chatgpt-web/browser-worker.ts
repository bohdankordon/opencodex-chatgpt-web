...
export async function setChatGptThinkMode(
  ...
  if (pressed !== target) {
    ...
    const composerState = () => composer.evaluate(element => {
      ...
      // Think draft integrity recognizes both the legacy selected-connector
      // pill and the new upstream 6.1 app-mention pill, using the same
      // ownership semantics accepted elsewhere (app:// path, display name,
      // non-editable pill). Anything else stays draft text and fails closed.
