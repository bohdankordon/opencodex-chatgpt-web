...
function strippedRoutedUserRevisions(
  ...
  if (parsed._externalProviderTrusted !== true) return [];
  const input = inputItems(parsed);
  ...
  for (const value of input) {
    const item = record(value);
    if (!isNativeInstruction(item, metadata, { allowMissingAgentMessageId: true })) continue;
...
export function chatGptTurnUserRevisionHistory(parsed: CodexParsedRequest): ChatGptTurnUserRevision[] {
  ...
  const stripped = turnId ? strippedRoutedUserRevisions(parsed, turnId) : [];
...
function canonicalMetadataEnvironmentBeforeUser(
  ...
  const user = record(input[userIndex]);
  if (!isNativeInstruction(user, metadata, instructionOptions)) return undefined;
  const userHasId = typeof user.id === "string" && user.id.length > 0;
  if (!userHasId && (requireMetadataBoundRoots || instructionOptions.allowMissingAgentMessageId !== true)) return undefined;
  const userTurnId = itemTurnId(user);
  if (userTurnId !== undefined && userTurnId !== metadataTurnId) return undefined;

  return canonicalMetadataEnvironmentBefore(input, userIndex, metadata, requireMetadataBoundRoots, instructionOptions);
}
...
function canonicalMetadataEnvironmentBefore(
  ...
  while (candidate?.type === "message" && (candidate.role === "developer" || compactionSummaryMessage(candidate))) {
    ...
    if (developerTurnId === undefined
      ? (requireMetadataBoundRoots || instructionOptions.allowMissingAgentMessageId !== true) && !serverOwnedId
  ...
  if (!candidateHasId && (requireMetadataBoundRoots || instructionOptions.allowMissingAgentMessageId !== true)) return undefined;
  const candidateTurnId = itemTurnId(candidate);
...
function strippedRoutedEnvironmentBeforeUser(
  ...
  if (userIndex <= 0 || !metadata || input.some(itemHasTurnProvenance)) return undefined;
  const metadataTurnId = typeof metadata.turn_id === "string" ? metadata.turn_id.trim() : "";
  const metadataSandbox = sandboxTypeFromMetadata(canonicalSandboxMetadata(metadata));
  if (!metadataTurnId || !metadataSandbox) return undefined;
  const user = record(input[userIndex]);
  if (!isNativeInstruction(user, metadata, { allowMissingAgentMessageId: true })) return undefined;
...
function rawEnvironmentText(parsed: CodexParsedRequest): string | undefined {
  ...
  const trustedStrippedTransport = parsed._externalProviderTrusted === true && !input.some(itemHasTurnProvenance);
  const instructionOptions = trustedStrippedTransport ? { allowMissingAgentMessageId: true } : {};
  for (let index = input.length - 1; index >= 0; index -= 1) {
    const item = record(input[index]);
    if (isNativeInstruction(item, metadata, instructionOptions)) {
  ...
  const current = checkpoint && metadata
    ...
    : canonicalMetadataEnvironmentBeforeUser(input, activeUserIndex, metadata, false, instructionOptions);
  if (current) return current;
  const stripped = trustedStrippedTransport
    ? strippedRoutedEnvironmentBeforeUser(input, activeUserIndex, metadata)
    : undefined;
